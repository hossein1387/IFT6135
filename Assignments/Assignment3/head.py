...
class NTMHeadBase(nn.Module):
    ...
    def _address_memory(self, k, beta, g, s, Y, w_prev):
        # Handle Activations
        k = k.clone()
        ...
        w = self.memory.address(k, beta, g, s, Y, w_prev)
...
class NTMReadHead(NTMHeadBase):
    def __init__(self, memory, controller_size):
        ...
        # Corresponding to k, beta, g, s, Y sizes from the paper
    ...
    def forward(self, embeddings, w_prev):
        ...
        k, beta, g, s, Y = _split_cols(o, self.read_lengths)

        # Read from memory
        w = self._address_memory(k, beta, g, s, Y, w_prev)
...
class NTMWriteHead(NTMHeadBase):
    ...
    def forward(self, embeddings, w_prev):
        ...
        k, beta, g, s, Y, e, a = _split_cols(o, self.write_lengths)
